...
# # Monitor CPU and GPU, Load into HEAVYAI
#
...
logging.basicConfig()
log = logging.getLogger("heavyai_monitor")
log.setLevel("DEBUG")
...
def all_metrics(con):
    ...
    db = heavyai_olio.ibis.db_memory(con.con, detail=0)

    for i, row in db.iterrows():
        df["db_" + row.device_type + "_mem_alloc_kb"] = int(
            row.page_size * row.pages_allocated / 1024
        )
        df["db_" + row.device_type + "_mem_used_kb"] = int(
            row.page_size * row.used_pages / 1024
        )
...
summary_table = "heavyai_system_metrics_summary"
...
def create_tables(tgt):
    ...
    # alter table heavyai_system_metrics_summary add container text
...
def monitor_import(sleep_seconds=1, batch=100, tgt_file=None):
    ...
    errors = 0
    while True:
        with heavyai_olio.ibis.connect(os.environ["HEAVYAI_DB_URL"]) as src:
            while True:
                ...
                try:
                    ...
                    if len(df) >= batch:
                        ...
                        if "HEAVYAI_DB_URL_TGT" in os.environ:
                            with heavyai_olio.ibis.connect(
                                os.environ["HEAVYAI_DB_URL_TGT"]
                            ) as tgt:
                                create_tables(tgt)
                                tgt.load_data(summary_table, df)
                                t = tgt.table(summary_table)
                                log.info(
                                    f"Loaded {len(df)}, count={t.count().execute()}"
                                )
                        df = None
                        errors = 0
                    else:
