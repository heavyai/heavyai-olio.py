...
def lookup_url_for_name(name):
    ...
    for fname in [
        home + "/heavyai/heavyai-urls.csv",
        home + "/.heavyai/heavyai-urls.csv",
        home + "/host_home/heavyai/heavyai-urls.csv",
        home + "/host_home/.heavyai/heavyai-urls.csv",
        home + "/shared_notebooks/heavyai/heavyai-urls.csv",
    ]:
        if os.path.exists(fname):
            ...
            if not s.empty:
                return s.values[0]
    raise Exception("HEAVYAI connection url not found for name {}".format(name))


def param_prompt(name, default_val, pw=False):
    if not pw:
        return input("HEAVY DB {} [{}]: ".format(name, default_val)) or default_val
    elif default_val is None:
        print("HEAVY DB password: ", end="")
...
def url_prompt(
    ...
    """
    url: 'heavydb://admin:HyperInteractive@heavyai.example.com:6274/heavyai?execution_type=3&protocol=binary'
    url: 'heavydb://admin@heavyai.example.com:6274/heavyai?ipc=gpu'
    """
    ...
    if type(url) == URL:
        ...
        u = make_url(os.environ["HEAVYAI_DB_URL"])
    else:
        u = URL("heavydb")
    ...
    if host:
        ...
        u.host = param_prompt("host", "localhost")

    if port:
        u.port = port
    elif not u.port:
        u.port = int(param_prompt("port", "6274"))

    if database:
        u.database = database
    elif not u.database:
        u.database = param_prompt("database", "heavyai")

    if protocol:
        u.query["protocol"] = protocol
    elif not u.query.get("protocol"):
        u.query["protocol"] = "binary"

    ipc_dict = {"cpu": "1", "gpu": "2", "remote": "3", None: "3"}
    if ipc:
        u.query["execution_type"] = ipc_dict[ipc]
    elif u.query.get("ipc"):
        u.query["execution_type"] = ipc_dict[u.query["ipc"]]
        del u.query["ipc"]

    if execution_type:
        u.query["execution_type"] = execution_type
...
def connect_prompt(
    ...
    return heavyai.connect(u)


def connect():
    return heavyai.connect(os.environ["HEAVYAI_DB_URL"])
