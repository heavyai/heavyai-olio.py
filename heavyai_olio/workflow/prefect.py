...
class HeavyaiStorageTask(Task):
    """
    Abstract Prefect Task to produce a SQL query operation (by a subclass)
    and store the results in a HEAVYAI DB table.
    """
...
class HeavyaiStorageLoopTask(HeavyaiStorageTask):
    """
    Abstract Prefect Task to produce a SQL query operation (by a subclass)
    and store the results in a HEAVYAI DB table
    ...
    def run(self, con_url, sources, target, forward_target=None, **kwargs):
        ...
        con_url - HEAVYAI DB connection URL
