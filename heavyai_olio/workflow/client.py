...
from heavyai_olio.ibis import connect as ibis_connect
...
def logger():
    ...
    # prefect's get_run_logger() will throw an exception if the current thread
    # is not running in a prefect context
    except:
...
def _schema_apply_to(schema, df):
    ...
    With HEAVYAI, input validate is more strict, so "1.0" fails to load as an INT.
...
def clean_name(name, pretty=False):
    from sqlalchemy_heavyai.base import RESERVED_WORDS
...
db_update_log_table = sc.Table(
    ...
    [
        ...
        sc.Column("heavyai_version", sc.text8),
        sc.Column("heavyai_session", sc.text32),
...
class HeavyDBClient:
    ...
    def __init__(
        ...
        if con is not None:
            ...
            uri = uri or os.environ.get("HEAVYAI_DB_URL")
            if uri is None:
                raise Exception(
                    "A DB connection URL must be provided by one of: `con`, `uri` param, or env var `HEAVYAI_DB_URL`"
                )
        ...
        if _other is not None:
            ...
            log_uri = log_uri or os.environ.get("HEAVYAI_DB_LOG_URL", None)
    ...
    def __enter__(self):
        ...
        # return HeavyDBClient(_other=self, close_on_exit=self.close_on_exit)
        return self
    ...
    def src_table(self, t):
        if isinstance(t, ibis_heavyai.client.HeavyDBTable):
    ...
    def to_expr(self, x):
        if isinstance(x, ibis_heavyai.client.HeavyDBTable):
    ...
    def _name(self, thing):
        if isinstance(thing, ibis_heavyai.client.HeavyDBTable):
            return thing.name
    ...
    def _log_init(self):
        if (not self._log_inited and self._log_con):
            ...
            with HeavyDBClient(con=self._log_con, log_uri=self._log_con, close_on_exit=False, default_severity=self.default_severity) as log_con:
...
def connect(con=None, close_on_exit=True, default_severity=None):
    ...
    def ctas_something(con, input_table_name, load_table=None):
        ...
        with heavyai_olio.workfow.connect(con) as con:
    ...
    Args:
        - con (URL string, Ibis connection, HeavyDBClient, or None to use env var HEAVYAI_DB_URL)
    ...
    Returns:
        HeavyDBClient: with a Ibis con and heavyai con.con, connected to HEAVYAI DB
    """
    if con is None or isinstance(con, str):
        return HeavyDBClient(uri=con, close_on_exit=close_on_exit, default_severity=default_severity)
    elif isinstance(con, HeavyDBClient):
        # return con
        return HeavyDBClient(_other=con, close_on_exit=False, default_severity=default_severity)
    elif isinstance(con, HeavyDBBackend):
        return HeavyDBClient(con=con, close_on_exit=False, default_severity=default_severity)
    else:
