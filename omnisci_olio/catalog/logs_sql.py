...
re_sql = re.compile(r"""sql_execute [0-9]+ ([0-9]+) (.*) .* .* {"query_str"} {"(.*)"}""", re.MULTILINE|re.DOTALL)
...
re_vega = re.compile(r"""render_vega [0-9]+ ([0-9]+) (.*) .* .* {"widget_id","compression_level","vega_json","nonce"} {"[0-9]+","[0-9]+","(.*)","[0-9]+"}""", re.MULTILINE|re.DOTALL)
...
def read_log_file_sql(log_path, load_label, quit_on_error = True, max_queries = 1000000, max_lines = 20000000):
    ...
    with open(log_path, 'r') as input:
        ...
        for line in input:
            ...
            if len(lines) == 1 and (line.find('sql_execute') == -1 and line.find('render_vega') == -1):
                # print('continue', line)
                continue
            # else:
